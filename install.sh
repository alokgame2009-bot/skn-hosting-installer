...
panel_menu() {
  while true; do
    ...
    printf "${GRAY}  SYSTEM STATUS${RESET}\n"
    printf "  ├─ Uptime : %s\n" "$(uptime -p 2>/dev/null | sed 's/^up //')"
    printf "  └─ Load   : %s\n" "$(awk '{print $1}' /proc/loadavg 2>/dev/null || echo '0.00')"
    echo -e "${VIOLET}────────────────────────────────────────────────────────────${RESET}"
    echo -e "${WHITE}${BOLD}    AVAILABLE DEPLOYMENTS${RESET}"
    echo -e "${VIOLET}  ┌──────────────────────────┬──────────────────────────┐${RESET}"
    printf "${VIOLET}  │${RESET} ${GREEN}[1]${RESET} Ptero                ${VIOLET}│${RESET} ${PINK}[7]${RESET} Convoy              ${VIOLET}│${RESET}\n"
    printf "${VIOLET}  │${RESET} ${GREEN}[2]${RESET} NovaStudio           ${VIOLET}│${RESET} ${PINK}[8]${RESET} FeatherPanel        ${VIOLET}│${RESET}\n"
    printf "${VIOLET}  │${RESET} ${GREEN}[3]${RESET} JexPanel             ${VIOLET}│${RESET} ${PINK}[9]${RESET} Mythicaldash         ${VIOLET}│${RESET}\n"
    printf "${VIOLET}  │${RESET} ${GREEN}[4]${RESET} Reviactyl            ${VIOLET}│${RESET} ${PINK}[10] Mythicaldashv3      ${VIOLET}│${RESET}\n"
    printf "${VIOLET}  │${RESET} ${GREEN}[5]${RESET} CtrlPanel            ${VIOLET}│${RESET} ${PINK}[11] VPS Panel           ${VIOLET}│${RESET}\n"
    printf "${VIOLET}  │${RESET} ${GREEN}[6]${RESET} Paymenter            ${VIOLET}│${RESET} ${RED}[0] Exit                 ${VIOLET}│${RESET}\n"
    echo -e "${VIOLET}  └──────────────────────────┴──────────────────────────┘${RESET}"
    echo
    printf "${GOLD}${BOLD}  λ Select Module [1-11]: ${RESET}"
    read -r x
    case "$x" in
      1) panel_actions ;;
      2) panel_module_info "NovaStudio" ;;
      3) panel_module_info "JexPanel" ;;
      4) panel_module_info "Reviactyl" ;;
      5) panel_module_info "CtrlPanel" ;;
      6) panel_module_info "Paymenter" ;;
      7) panel_module_info "Convoy" ;;
      8) panel_module_info "FeatherPanel" ;;
      9) panel_module_info "Mythicaldash" ;;
      10) panel_module_info "Mythicaldashv3" ;;
      11) panel_module_info "VPS Panel" ;;
      0) return ;;
...
panel_module_info() {
  local name="$1"
  clear_screen
  echo -e "${CYAN}${BOLD}SERVER PANEL MODULE — ${name}${RESET}"
  echo
  echo -e "${GRAY}Module entry added to the SKN HOSTING manager.${RESET}"
  echo -e "${GRAY}Use the panel's official installation method for deployment.${RESET}"
  pause
}
...
panel_actions() {
  while true; do
    clear_screen
    echo -e "${CYAN}${BOLD}PTERODACTYL PANEL MANAGER${RESET}"
    echo "[1] Install Panel"
    echo "[2] Install Wings"
    echo "[3] Install Panel + Wings"
    echo "[4] Update Panel"
    echo "[5] Panel Configuration"
    echo "[6] Clear Panel Cache"
    echo "[7] Restart Panel Queue"
    echo "[0] Back"
    echo
    read -r -p "➜ Pterodactyl option: " x
    case "$x" in
      1) install_panel; pause ;;
      2) install_wings; pause ;;
      3) install_both; pause ;;
...
menu() {
  ...
  system_status
  echo
  echo -e "${CYAN}${BOLD}   DEPLOYMENT & SERVICES${RESET}"
  printf "  ├─ ${GREEN}[1]${RESET} Panel              ├─ ${PINK}[5]${RESET} Themes\n"
  printf "  ├─ ${GREEN}[2]${RESET} Wings              ├─ ${PINK}[6]${RESET} System\n"
  printf "  ├─ ${GOLD}[3]${RESET} Toolbox            ├─ ${PINK}[7]${RESET} Container\n"
  printf "  └─ ${GOLD}[4]${RESET} Extras             └─ ${PINK}[8]${RESET} New Module\n"
  echo
  echo -e "${WHITE}${BOLD}   MAINTENANCE & TOOLS${RESET}"
  printf "    ${RED}[9]${RESET} Extras                             ${RED}[0]${RESET} SHUTDOWN SYSTEM\n"
  echo -e "${VIOLET}  ──────────────────────────────────────────────────────────────────────────${RESET}"
