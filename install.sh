#!/usr/bin/env bash
set -Ee
# Intentionally do not use `set -u`: optional UI/color variables must never abort the installer.
...
# Safe color fallbacks: keep the installer usable even if an environment
# or shell wrapper has unset one of the color variables.
: "${RESET:=${ESC}[0m}"
: "${BOLD:=${ESC}[1m}"
: "${RED:=${ESC}[1;38;5;196m}"
: "${GREEN:=${ESC}[1;38;5;82m}"
...
menu() {
  ...
  printf "${GRAY}    ├─ ${GREEN}[1]${RESET} ${GREEN}Panel${RESET}             ${GRAY}├─ ${YELLOW}[5]${RESET} ${GREEN}Themes${RESET}\n"
  printf "${GRAY}    ├─ ${GREEN}[2]${RESET} ${GREEN}Wings${RESET}             ${GRAY}├─ ${YELLOW}[6]${RESET} ${GREEN}System${RESET}\n"
  printf "${GRAY}    ├─ ${GREEN}[3]${RESET} ${GREEN}Toolbox${RESET}           ${GRAY}├─ ${YELLOW}[7]${RESET} ${GREEN}Container${RESET}\n"
  printf "${GRAY}    └─ ${GREEN}[4]${RESET} ${GREEN}Extras${RESET}            ${GRAY}└─ ${YELLOW}[8]${RESET} ${GREEN}New Module${RESET}\n"
  echo
...
panel_menu() {
  while true; do
    header; echo; echo -e "${CYAN}${BOLD}  🛰️  SERVER PANEL MANAGER v15.0${RESET}"
    ...
    echo -e "${CYAN}  ┌──────────────────────────┬──────────────────────────┐${RESET}"
    printf "${CYAN}  │${RESET} ${GREEN}[1]${RESET} ${GREEN}Pterodactyl${RESET}        ${CYAN}│${RESET} ${YELLOW}[4]${RESET} ${GREEN}CtrlPanel${RESET}          ${CYAN}│${RESET}\n"
    printf "${CYAN}  │${RESET} ${GREEN}[2]${RESET} ${GREEN}JexPanel${RESET}            ${CYAN}│${RESET} ${YELLOW}[5]${RESET} ${GREEN}Paymenter${RESET}          ${CYAN}│${RESET}\n"
    printf "${CYAN}  │${RESET} ${GREEN}[3]${RESET} ${GREEN}Reviactyl${RESET}           ${CYAN}│${RESET} ${YELLOW}[6]${RESET} ${GREEN}FeatherPanel${RESET}       ${CYAN}│${RESET}\n"
    printf "${CYAN}  │${RESET} ${RED}[7]${RESET} ${GREEN}Mythicaldash${RESET}        ${CYAN}│${RESET} ${RED}[8]${RESET} ${GREEN}Mythicaldashv3${RESET}      ${CYAN}│${RESET}\n"
    printf "${CYAN}  │${RESET} ${WHITE}[0] Exit${RESET}              ${CYAN}│${RESET}                          ${CYAN}│${RESET}\n"
    echo -e "${CYAN}  └──────────────────────────┴──────────────────────────┘${RESET}"; echo
    printf "${GOLD}${BOLD}  λ Select Module [1-8]: ${RESET}"; read -r x
    case "$x" in 1) ptero_manager;; 2) jexactyl_manager;; 3) reviactyl_manager;; 4) ctrlpanel_manager;; 5) paymenter_manager;; 6) feather_manager;; 7) mythicaldash_manager;; 8) mythicaldash_v3_manager;; 0) return;; *) echo -e "${RED}✗ Invalid module.${RESET}"; sleep 1;; esac
  done
}

skn_panel_config() {
  mkdir -p /etc/skn-hosting; local f=/etc/skn-hosting/panel.conf
  [[ -f "$f" ]] && return 0
  clear_screen; echo -e "${CYAN}${BOLD}SKN HOSTING — FIRST TIME PANEL SETUP${RESET}"; echo
  read -r -p "  Panel domain: " SKN_DOMAIN
  read -r -p "  Admin email: " SKN_EMAIL
  read -r -p "  Admin username [admin]: " SKN_ADMIN_USER; SKN_ADMIN_USER="${SKN_ADMIN_USER:-admin}"
  while :; do
    read -r -s -p "  Admin password (8+ chars, upper/lower + number): " SKN_ADMIN_PASS; echo
    [[ ${#SKN_ADMIN_PASS} -ge 8 && "$SKN_ADMIN_PASS" =~ [A-Z] && "$SKN_ADMIN_PASS" =~ [a-z] && "$SKN_ADMIN_PASS" =~ [0-9] ]] && break
    echo -e "${RED}  Password does not meet the requirements.${RESET}"
  done
  read -r -p "  Timezone [Asia/Kolkata]: " SKN_TZ; SKN_TZ="${SKN_TZ:-Asia/Kolkata}"
  cat > "$f" <<EOF
SKN_DOMAIN=$(printf '%q' "$SKN_DOMAIN")
SKN_EMAIL=$(printf '%q' "$SKN_EMAIL")
SKN_ADMIN_USER=$(printf '%q' "$SKN_ADMIN_USER")
SKN_ADMIN_PASS=$(printf '%q' "$SKN_ADMIN_PASS")
SKN_TZ=$(printf '%q' "$SKN_TZ")
EOF
  chmod 600 "$f"; echo -e "${GREEN}✓ Details saved. Future panel actions reuse them.${RESET}"; sleep 1
}
...
panel_loading() {
  local name="$1" frames='⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏' i
  echo -e "${CYAN}  ➜ Executing ${name} Routine...${RESET}"
  for i in {0..19}; do printf '\r  %s%s%s [%02d/20] Loading %s...' "$CYAN" "${frames:i%10:1}" "$RESET" "$((i+1))" "$name"; sleep 0.05; done
  printf '\r\033[K'
}
...
panel_shell_menu() {
  ...
  while true; do
    clear_screen; panel_status "$name"; echo
    echo -e "${WHITE}${BOLD}  ┌───────────────────────────────────────────────────────┐${RESET}"
    printf "${WHITE}  │${RESET}  ${GREEN}%s MANAGER${RESET}                                  ${WHITE}│${RESET}\n" "$name"
    echo -e "${WHITE}  ├───────────────────────────────────────────────────────┤${RESET}"
    printf "${WHITE}  │${RESET}  ${GREEN}[1]${RESET} ${GREEN}Install${RESET}       :: (Fresh Install)                ${WHITE}│${RESET}\n"
    printf "${WHITE}  │${RESET}  ${GREEN}[2]${RESET} ${GREEN}Admin/User${RESET}    :: (Initial Account)              ${WHITE}│${RESET}\n"
    printf "${WHITE}  │${RESET}  ${YELLOW}[3]${RESET} ${GREEN}Update${RESET}        :: (Latest Release)               ${WHITE}│${RESET}\n"
    printf "${WHITE}  │${RESET}  ${YELLOW}[4]${RESET} ${GREEN}Domain/SSL${RESET}    :: (Domain / SSL)                 ${WHITE}│${RESET}\n"
    printf "${WHITE}  │${RESET}  ${RED}[5]${RESET} ${GREEN}Uninstall${RESET}     :: (Remove Panel Data)            ${WHITE}│${RESET}\n"
    printf "${WHITE}  │${RESET}  ${RED}[6]${RESET} ${GREEN}phpMyAdmin${RESET}    :: (Database Tool)                ${WHITE}│${RESET}\n"
    printf "${WHITE}  │${RESET}  ${WHITE}[0]${RESET} ${GREEN}Exit${RESET}          :: (Back to Panel Manager)        ${WHITE}│${RESET}\n"
    echo -e "${WHITE}  └───────────────────────────────────────────────────────┘${RESET}"; echo
    printf "${GOLD}${BOLD}  ➜ Select ${name} Option [0-6]: ${RESET}"; read -r x
    case "$x" in 1) panel_loading "$name"; load_panel_config; "$install_fn"; pause;; 2) panel_loading "$name"; load_panel_config; "$user_fn"; pause;; 3) panel_loading "$name"; "$update_fn"; pause;; 4) panel_loading "$name"; load_panel_config; "$domain_fn"; pause;; 5) panel_loading "$name"; "$uninstall_fn"; pause;; 6) panel_loading "phpMyAdmin"; phpmyadmin_tools; pause;; 0) return;; *) echo -e "${RED}✗ Invalid option.${RESET}"; sleep 1;; esac
  done
}

panel_status() {
  local name="$1" path=""
  case "$name" in Pterodactyl) path="$PTERODACTYL_DIR";; Jexactyl) path="/var/www/jexactyl";; Reviactyl) path="/var/www/reviactyl";; CtrlPanel) path="/var/www/ctrlpanel";; Paymenter) path="/var/www/paymenter";; FeatherPanel) path="/opt/featherpanel";; MythicalDash|MythicalDashV3) path="/var/www/mythicaldash";; esac
  [[ -n "$path" && -d "$path" ]] && echo -e "${GREEN}${BOLD}  PANEL STATUS: INSTALLED ✓${RESET}" || echo -e "${RED}${BOLD}  PANEL STATUS: NOT INSTALLED ✘${RESET}"
}

ptero_manager() { panel_shell_menu "Pterodactyl" install_panel ptero_user_menu update_panel ptero_domain ptero_uninstall; }
ptero_do() { ptero_manager "$@"; }
ptero_versions=("v1.15.1" "v1.15.0" "v1.14.1" "v1.14.0" "v1.13.0" "v1.12.4" "v1.12.3" "v1.12.2" "v1.12.1" "v1.12.0" "v1.11.11" "v1.11.10" "v1.11.9" "v1.11.8" "v1.11.7" "v1.11.6" "v1.11.5" "v1.11.4" "v1.11.3" "v1.11.2")

ptero_install_config() {
  load_panel_config; clear_screen
  cat <<'ART'
...
────────────────────────────────────────────────────────────
ART
  echo -e "  ${WHITE}• Panel Domain${RESET} [${SKN_DOMAIN}]"; echo -e "  ${WHITE}╰─>${RESET} saved configuration"
  echo -e "  ${WHITE}• Admin Email${RESET} [${SKN_EMAIL}]"; echo -e "  ${WHITE}╰─>${RESET} saved configuration"
  echo -e "  ${WHITE}• Admin Username${RESET} [${SKN_ADMIN_USER}]"; echo -e "  ${WHITE}╰─>${RESET} saved configuration"
  echo -e "  ${WHITE}• Admin Password${RESET} [********]"; echo -e "  ${WHITE}╰─>${RESET} saved securely"; echo
  echo -e "  ${WHITE}:: Available Panel Versions${RESET}"
  local i n color choice version
  for i in "${!ptero_versions[@]}"; do n=$((i+1)); if ((n<=3)); then color="$GREEN"; elif ((n<=6)); then color="$YELLOW"; else color="$RED"; fi; printf "  ${color}%2d.${RESET} ${GREEN}%s${RESET}\n" "$n" "${ptero_versions[$i]}"; done
  echo; printf "  ${WHITE}• Select version [1-20] [1 = latest]${RESET}\n  ${GOLD}╰─>${RESET} "; read -r choice
  [[ "$choice" =~ ^[0-9]+$ ]] && ((choice>=1 && choice<=20)) || { echo -e "${RED}Invalid version.${RESET}"; return 1; }
  version="${ptero_versions[$((choice-1))]}"; echo -e "  ${GREEN}→ ${version}${RESET}"; echo
  echo -e "  ${WHITE}┌─[ REVIEW CONFIGURATION ]${RESET}"; echo -e "  ${WHITE}│ Domain:   ${SKN_DOMAIN}${RESET}"; echo -e "  ${WHITE}│ Email:    ${SKN_EMAIL}${RESET}"; echo -e "  ${WHITE}│ User:     ${SKN_ADMIN_USER}${RESET}"; echo -e "  ${WHITE}│ Version:  ${version}${RESET}"; echo -e "  ${WHITE}└───────────────────────────${RESET}"; echo
  while true; do read -r -p "  Start Installation? (y/n): " yn; case "$yn" in y|Y) break;; n|N) echo -e "${YELLOW}Installation cancelled.${RESET}"; return 0;; *) echo -e "${RED}Invalid input. Enter y or n.${RESET}";; esac; done
  ptero_install_selected "$version"
}

ptero_preflight() {
  echo -e "${CYAN}${BOLD}  PRE-FLIGHT CHECK${RESET}"
  local os arch virt ip
  os="$(. /etc/os-release 2>/dev/null; echo "${PRETTY_NAME:-unknown}")"
  arch="$(uname -m 2>/dev/null || echo unknown)"
  virt="$(systemd-detect-virt 2>/dev/null || echo unknown)"
  ip="$(curl -4fsS --max-time 5 https://api.ipify.org 2>/dev/null || true)"
  [[ -n "$ip" ]] || ip="not detected"
  printf "  ${WHITE}OS        :${RESET} %s\\n" "$os"
  printf "  ${WHITE}Architecture:${RESET} %s\\n" "$arch"
  printf "  ${WHITE}Virtualization:${RESET} %s\\n" "$virt"
  printf "  ${WHITE}Public IPv4:${RESET} ${GREEN}%s${RESET}\\n" "$ip"
  if [[ "$virt" == "kvm" || "$virt" == "qemu" || "$virt" == "vmware" || "$virt" == "oracle" || "$virt" == "microsoft" || "$virt" == "xen" || "$virt" == "none" || "$virt" == "unknown" ]]; then
    echo -e "  ${GREEN}✓ Environment accepted for panel installation.${RESET}"
  else
    echo -e "  ${YELLOW}⚠ Virtualization type: $virt — continuing.${RESET}"
  fi
  echo
}

ptero_install_selected() {
  local version="$1"; require_debian || return 1
  [[ -d "$PTERODACTYL_DIR" ]] && { echo -e "${YELLOW}Pterodactyl already exists. Use Update.${RESET}"; return 1; }
  ptero_preflight
  echo -e "${CYAN}  ➜ Preparing Pterodactyl ${version} installation...${RESET}"
  run_animated "Updating package index" apt-get update -y || return 1
  run_animated "Installing web/database dependencies" apt-get install -y ca-certificates curl tar unzip git nginx mariadb-server redis-server openssl php8.3 php8.3-common php8.3-cli php8.3-gd php8.3-mysql php8.3-mbstring php8.3-bcmath php8.3-xml php8.3-fpm php8.3-curl php8.3-zip php8.3-intl php8.3-redis || return 1
  run_animated "Starting MariaDB, Redis and Nginx" bash -c 'systemctl enable --now mariadb redis-server nginx'
  if ! command -v composer >/dev/null 2>&1; then
    run_animated "Installing Composer" bash -c 'curl -fsSL https://getcomposer.org/installer -o /tmp/composer.php && php /tmp/composer.php --install-dir=/usr/local/bin --filename=composer && rm -f /tmp/composer.php' || return 1
  fi
  local dbpass; dbpass="$(openssl rand -hex 20)"; mkdir -p "$PTERODACTYL_DIR"; cd "$PTERODACTYL_DIR" || return 1
  run_animated "Downloading Pterodactyl ${version}" curl -fL "https://github.com/pterodactyl/panel/releases/download/${version}/panel.tar.gz" -o panel.tar.gz || return 1
  run_animated "Extracting Pterodactyl files" tar -xzf panel.tar.gz
  rm -f panel.tar.gz; chmod -R 755 storage bootstrap/cache; cp .env.example .env
  run_animated "Installing PHP dependencies" env COMPOSER_ALLOW_SUPERUSER=1 composer install --no-dev --optimize-autoloader --no-interaction || return 1
  run_animated "Generating application key" php artisan key:generate --force || return 1
  run_animated "Configuring application environment" php artisan p:environment:setup -n --author="$SKN_EMAIL" --url="https://${SKN_DOMAIN}" --timezone="$SKN_TZ" --cache=redis --session=database --queue=redis --redis-host=127.0.0.1 --redis-pass= --redis-port=6379 --telemetry=true || return 1
  mariadb -u root <<SQL
...
GRANT ALL PRIVILEGES ON panel.* TO 'pterodactyl'@'127.0.0.1'; FLUSH PRIVILEGES;
SQL
  run_animated "Configuring database" php artisan p:environment:database -n --host=127.0.0.1 --port=3306 --database=panel --username=pterodactyl --password="$dbpass" || return 1
  run_animated "Running database migrations" php artisan migrate --seed --force || return 1
  run_animated "Creating administrator account" php artisan p:user:make --email="$SKN_EMAIL" --username="$SKN_ADMIN_USER" --name-first="SKN" --name-last="Admin" --password="$SKN_ADMIN_PASS" --admin=1 || return 1
  chown -R www-data:www-data "$PTERODACTYL_DIR"
  cat > /etc/nginx/sites-available/pterodactyl.conf <<NGINX
server { listen 80; server_name ${SKN_DOMAIN}; root ${PTERODACTYL_DIR}/public; index index.php; client_max_body_size 100G;
location / { try_files \$uri \$uri/ /index.php?\$query_string; }
location ~ \.php$ { include snippets/fastcgi-php.conf; fastcgi_pass unix:/run/php/php8.3-fpm.sock; }
location ~ /\.ht { deny all; }
}
NGINX
  ln -sf /etc/nginx/sites-available/pterodactyl.conf /etc/nginx/sites-enabled/pterodactyl.conf; rm -f /etc/nginx/sites-enabled/default
  nginx -t && systemctl reload nginx || return 1
  (crontab -u www-data -l 2>/dev/null | grep -Fv '/var/www/pterodactyl/artisan schedule:run' || true; echo '* * * * * php /var/www/pterodactyl/artisan schedule:run >> /dev/null 2>&1') | crontab -u www-data -
...
UNIT
  ...
  if grep -q '^SKN_PTERO_VERSION=' /etc/skn-hosting/panel.conf 2>/dev/null; then
    sed -i '/^SKN_PTERO_VERSION=/d' /etc/skn-hosting/panel.conf
  fi
  echo "SKN_PTERO_VERSION=$(printf '%q' "$version")" >> /etc/skn-hosting/panel.conf
  echo -e "${GREEN}${BOLD}✓ Pterodactyl ${version} installation completed.${RESET}"
  echo -e "${GRAY}  Panel URL: https://${SKN_DOMAIN}${RESET}"
}

install_panel() { ptero_install_config; }

update_panel() {
  [[ -f "$PTERODACTYL_DIR/artisan" ]] || { echo -e "${RED}Pterodactyl is not installed.${RESET}"; return 1; }
  load_panel_config; local version="${SKN_PTERO_VERSION:-v1.15.1}" requested
  read -r -p "Update to version [${version}] or enter another tag: " requested; version="${requested:-$version}"
  cd "$PTERODACTYL_DIR"; php artisan down || true
  curl -fL "https://github.com/pterodactyl/panel/releases/download/${version}/panel.tar.gz" -o /tmp/panel-update.tar.gz || { php artisan up || true; return 1; }
  tar -xzf /tmp/panel-update.tar.gz; rm -f /tmp/panel-update.tar.gz; chmod -R 755 storage bootstrap/cache
  COMPOSER_ALLOW_SUPERUSER=1 composer install --no-dev --optimize-autoloader --no-interaction || { php artisan up || true; return 1; }
  php artisan migrate --seed --force; php artisan view:clear; php artisan config:clear; php artisan queue:restart; chown -R www-data:www-data "$PTERODACTYL_DIR"; php artisan up
  echo -e "${GREEN}✓ Pterodactyl ${version} updated.${RESET}"
}
...
ptero_user_menu() {
  load_panel_config; [[ -f "$PTERODACTYL_DIR/artisan" ]] || { echo -e "${RED}Pterodactyl is not installed.${RESET}"; return 1; }
  echo -e "${WHITE}Pterodactyl Admin/User${RESET}"; echo -e "${GRAY}Initial admin: ${SKN_ADMIN_USER} (${SKN_EMAIL})${RESET}"
  read -r -p "Create another user? (y/n): " yn; [[ "$yn" =~ ^[Yy]$ ]] || return 0
  read -r -p "Email: " email; read -r -p "Username: " user; read -r -p "First name: " first; read -r -p "Last name: " last; read -r -s -p "Password: " pass; echo
  read -r -p "Admin? (y/n): " adm; local admin=0; [[ "$adm" =~ ^[Yy]$ ]] && admin=1
  (cd "$PTERODACTYL_DIR" && php artisan p:user:make --email="$email" --username="$user" --name-first="$first" --name-last="$last" --password="$pass" --admin="$admin")
}

ptero_domain() {
  load_panel_config; require_debian || return 1; apt-get update -y; apt-get install -y certbot python3-certbot-nginx || return 1; nginx -t || return 1
  certbot --nginx -d "$SKN_DOMAIN" --non-interactive --agree-tos -m "$SKN_EMAIL" --redirect
}
...
ptero_uninstall() {
  echo -e "${RED}Pterodactyl uninstall is destructive.${RESET}"; read -r -p "Type REMOVE-PTERO to confirm: " c; [[ "$c" == "REMOVE-PTERO" ]] || return 0
  systemctl disable --now pteroq 2>/dev/null || true; rm -rf "$PTERODACTYL_DIR" /etc/pterodactyl /etc/systemd/system/pteroq.service /etc/nginx/sites-enabled/pterodactyl.conf /etc/nginx/sites-available/pterodactyl.conf
  systemctl daemon-reload; nginx -t && systemctl reload nginx || true; echo -e "${GREEN}✓ Pterodactyl removed.${RESET}"
}
