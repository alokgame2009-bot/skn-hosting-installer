...
EOF
  echo -e "${RESET}"
  echo -e "${VIOLET}╔══════════════════════════════════════════════════════════════════════════════╗${RESET}"
  printf "${VIOLET}║${RESET} ${PINK}${BOLD}  SKN HOSTING${RESET} ${GRAY}•${RESET} ${GOLD}${BOLD}VIP ELITE CONTROL${RESET} ${GRAY}•${RESET} ${CYAN}ALL-IN-ONE${RESET}"
  printf "%*s${VIOLET}║${RESET}\n" $((24)) ""
  echo -e "${VIOLET}║${RESET} ${GRAY}  Pterodactyl • Wings • Docker • Database • Network • System Tools${RESET}        ${VIOLET}║${RESET}"
  echo -e "${VIOLET}║${RESET} ${WHITE}  $(date '+%Y-%m-%d %H:%M:%S')${RESET} ${GRAY}|${RESET} ${GREEN}SKN UPLINK READY${RESET}                                      ${VIOLET}║${RESET}"
  echo -e "${VIOLET}╚══════════════════════════════════════════════════════════════════════════════╝${RESET}"
}
...
system_status() {
  ...
  echo -e " ${CYAN}◉ LIVE SYSTEM STATUS${RESET}"
  echo -e " ${GRAY}├─ CPU Usage       :${RESET} ${WHITE}${cpu}%${RESET}"
  echo -e " ${GRAY}├─ RAM Usage       :${RESET} ${WHITE}${ram}%${RESET}"
  echo -e " ${GRAY}├─ Disk Usage      :${RESET} ${WHITE}${disk}%${RESET}"
  echo -e " ${GRAY}└─ Network         :${RESET} ${net}"
}
...
menu() {
  ...
  echo
  echo -e "${VIOLET}╭──────────────────────────────────────────────────────────────────────────────╮${RESET}"
  echo -e "${VIOLET}│${RESET} ${PINK}${BOLD}PTERODACTYL CORE${RESET}                                                        ${VIOLET}│${RESET}"
  echo -e "${VIOLET}├──────────────────────────────────────────────────────────────────────────────┤${RESET}"
  printf "${VIOLET}│${RESET} ${GREEN}[01]${RESET} Install Panel       ${GREEN}[02]${RESET} Install Wings       ${GREEN}[03]${RESET} Panel + Wings          ${VIOLET}│\n"
  printf "${VIOLET}│${RESET} ${CYAN}[04]${RESET} Update Panel        ${CYAN}[05]${RESET} Update Wings        ${CYAN}[06]${RESET} Service Manager         ${VIOLET}│\n"
  echo -e "${VIOLET}├──────────────────────────────────────────────────────────────────────────────┤${RESET}"
  echo -e "${VIOLET}│${RESET} ${GOLD}${BOLD}SERVER TOOLS${RESET}                                                             ${VIOLET}│${RESET}"
  printf "${VIOLET}│${RESET} ${GOLD}[07]${RESET} Database            ${GOLD}[08]${RESET} Redis               ${GOLD}[09]${RESET} Docker                  ${VIOLET}│\n"
  printf "${VIOLET}│${RESET} ${NEON}[10]${RESET} Network             ${NEON}[11]${RESET} Firewall            ${NEON}[12]${RESET} Storage                 ${VIOLET}│\n"
  printf "${VIOLET}│${RESET} ${PINK}[13]${RESET} System              ${PINK}[14]${RESET} Logs                ${PINK}[15]${RESET} OS Manager              ${VIOLET}│\n"
  echo -e "${VIOLET}├──────────────────────────────────────────────────────────────────────────────┤${RESET}"
  echo -e "${VIOLET}│${RESET} ${CYAN}${BOLD}PANEL EXTENSIONS${RESET}                                                        ${VIOLET}│${RESET}"
  printf "${VIOLET}│${RESET} ${CYAN}[16]${RESET} Blueprint           ${CYAN}[17]${RESET} PHP / Composer      ${CYAN}[18]${RESET} Node.js                 ${VIOLET}│\n"
  printf "${VIOLET}│${RESET} ${GREEN}[19]${RESET} phpMyAdmin          ${GREEN}[20]${RESET} Queue Restart       ${GREEN}[21]${RESET} Cache Clear              ${VIOLET}│\n"
  echo -e "${VIOLET}├──────────────────────────────────────────────────────────────────────────────┤${RESET}"
  echo -e "${VIOLET}│${RESET} ${WHITE}${BOLD}MANAGEMENT${RESET}                                                              ${VIOLET}│${RESET}"
  printf "${VIOLET}│${RESET} ${WHITE}[22]${RESET} VPS / VM Manager    ${WHITE}[23]${RESET} Panel Config        ${WHITE}[24]${RESET} Node Config              ${VIOLET}│\n"
  printf "${VIOLET}│${RESET} ${GRAY}[25]${RESET} System Information  ${GRAY}[26]${RESET} Diagnostics         ${GRAY}[27]${RESET} Reboot                  ${VIOLET}│\n"
  echo -e "${VIOLET}├──────────────────────────────────────────────────────────────────────────────┤${RESET}"
  echo -e "${VIOLET}│${RESET} ${RED}[00]${RESET} Exit                  ${GRAY}SKN HOSTING • Made By Anik${RESET}                         ${VIOLET}│${RESET}"
  echo -e "${VIOLET}╰──────────────────────────────────────────────────────────────────────────────╯${RESET}"
  echo
  echo -ne "${GOLD}${BOLD}root@skn-hosting ➜ ${RESET}"
}
...
redis_tools() {
  echo -e "${REDIS_COLOR:-$PINK}REDIS MANAGER${RESET}"
...
main() {
  ...
  while true; do
    ...
    read -r option
    case "$option" in
      01|1) run_animated "Installing Pterodactyl Panel..." install_panel || true; pause ;;
      02|2) run_animated "Installing Wings..." install_wings || true; pause ;;
      03|3) run_animated "Installing Panel + Wings..." install_both || true; pause ;;
      04|4) run_animated "Updating Panel..." update_panel || true; pause ;;
      05|5) run_animated "Updating Wings..." update_wings || true; pause ;;
      06|6) service_manager ;;
      07|7) database_tools ;;
      08|8) redis_tools ;;
      09|9) docker_tools ;;
      10) network_tools ;;
      ...
      23) panel_config ;;
      24) node_config ;;
      25) system_information ;;
      26) diagnostics ;;
      27) read -r -p "Type REBOOT to confirm: " c; [[ "$c" == REBOOT ]] && reboot ;;
      00|0) clear_screen; echo -e "${GREEN}SKN HOSTING installer closed safely.${RESET}"; exit 0 ;;
