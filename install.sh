...
ptero_install_selected() {
  ...
  apt-get update -y; apt-get install -y ca-certificates curl tar unzip git nginx mariadb-server redis-server php8.3 php8.3-{common,cli,gd,mysql,mbstring,bcmath,xml,fpm,curl,zip} openssl || return 1
  systemctl enable --now mariadb redis-server nginx
  command -v composer >/dev/null 2>&1 || { curl -fsSL https://getcomposer.org/installer -o /tmp/composer.php; php /tmp/composer.php --install-dir=/usr/local/bin --filename=composer; rm -f /tmp/composer.php; }
  local dbpass; dbpass="$(openssl rand -hex 20)"; mkdir -p "$PTERODACTYL_DIR"; cd "$PTERODACTYL_DIR"
  echo -e "${CYAN}Downloading ${version}...${RESET}"; curl -fL "https://github.com/pterodactyl/panel/releases/download/${version}/panel.tar.gz" -o panel.tar.gz || return 1
  tar -xzf panel.tar.gz; rm -f panel.tar.gz; chmod -R 755 storage bootstrap/cache; cp .env.example .env
  COMPOSER_ALLOW_SUPERUSER=1 composer install --no-dev --optimize-autoloader --no-interaction || return 1
  php artisan key:generate --force || return 1
  php artisan p:environment:setup -n --author="$SKN_EMAIL" --url="https://${SKN_DOMAIN}" --timezone="$SKN_TZ" --cache=redis --session=database --queue=redis --redis-host=127.0.0.1 --redis-pass= --redis-port=6379 --telemetry=true || return 1
  mariadb -u root <<SQL
...
SQL
  php artisan p:environment:database -n --host=127.0.0.1 --port=3306 --database=panel --username=pterodactyl --password="$dbpass" || return 1
  php artisan migrate --seed --force || return 1
  php artisan p:user:make --email="$SKN_EMAIL" --username="$SKN_ADMIN_USER" --name-first="SKN" --name-last="Admin" --password="$SKN_ADMIN_PASS" --admin=1 || return 1
...
NGINX
  ln -sf /etc/nginx/sites-available/pterodactyl.conf /etc/nginx/sites-enabled/pterodactyl.conf; rm -f /etc/nginx/sites-enabled/default; nginx -t && systemctl reload nginx
...
UNIT
  ...
  echo -e "${GREEN}✓ Pterodactyl ${version} installed.${RESET}"
}
